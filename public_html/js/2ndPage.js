...
function loadData(jsonFile){
  ...
  function containElement(typeElement){
      return data.pagesCon[1].hasOwnProperty(typeElement);
  }

function addHeader(portfolioData){

   var para = document.createElement("h1");
   var node = document.createTextNode(data.pagesCon[1].h1);
...
function addVideo(portfolioData){
    ...
    sourceMP4.type = data.pagesCon[1].videoType;
    sourceMP4.src = data.pagesCon[1].videoPath;
...
// img.setAttribute('src', "ePortfolioBannerImage.png");
 img.setAttribute('src', data.pagesCon[1].bannerImagePath);
...
var node = document.createTextNode(data.pagesCon[1].p1);
para.appendChild(node);
...
function addListComp(portfolioData){
    ...
    for (var i = 0; i <data.pagesCon[1].List.length; i++){
        var newNumberListItem = document.createElement("li");
        var numberListValue = document.createTextNode(data.pagesCon[1].List[i]);
...
function addImageComponent(portfolioData){
    var img = document.createElement("img");
    img.setAttribute('src', data.pagesCon[1].imagePath);
